...
<html>
<head>
  <link rel="icon" href="wallpapers/pokeball_1.png" type="image/png" />
...
<body>
  ...
  <form action="{{route('pokemonDetails', $item->id)}}" method="POST">
    @csrf
      <div class="card" id="card-Container" style="background-color: <?php echo $item->t_color; ?>;">
        <p>
           <h4> # {{$item->pokemon_number}} </h4>
           <h4> {{$item->pokemon_name}} </h4>
        </p>
        <div align="center">
          <a href="{{route('pokemonDetails', $item->pokemon_number)}}" id="id"><img src="{{asset('img/pokemon/'.$item->pokemon_number.'.png')}}"></a>
        </div>
        <br>
        <label class="typeLabel" id="type"> {{$item->pokemon_primaryType}} {{$item->pokemon_secondaryType}} </label>
      </div>
   @endforeach
  </form>
...
<script>
  //Function to re-color card by pokemon type
  //document.getElementById('card-Container').style.background='#72DA5A';
  //var select = document.getElementById('type').innerHTML; //El <select>
  ...
  //Funciones para deshabilitar campos destinados a null
  function disableAbilitieOne(value){

      if(value==true){
        // Enabled
        document.getElementById("abDescription_2").disabled=true;
      }else if(value==false){
        // Disabled
        document.getElementById("abDescription_2").disabled=false;
      }
  }
