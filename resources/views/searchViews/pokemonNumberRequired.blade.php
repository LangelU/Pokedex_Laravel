...
<body>
  ...
  <div align="center" class="required_pokemon">
    @foreach ($pokemon as $item)
      <div class="card" style="background-color: <?php echo $item->color; ?>; border-radius: 10px 10px 10px 10px">
        <p>
           <h4> # {{$item->pokemon_number}} </h4>
           <h4> {{$item->pokemon_name}} </h4>
        </p>
        <div align="center">
          <a href="{{route('pokemonDetails', $item->pokemon_number)}}"><img src="{{asset('img/pokemon/'.$item->pokemon_number.'.png')}}"></a>
        </div>
